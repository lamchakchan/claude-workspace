...
| Persistent knowledge / memory | memory | (no built-in equivalent) |
...
- **Memory MCP** (` + "`mcp__memory__*`" + ` tools): Cross-project structured knowledge graph. NOT auto-loaded — call ` + "`search_nodes`" + ` at session start to load relevant context. Use for user preferences and patterns that span multiple projects. Clear in-session with ` + "`delete_entities`" + ` or by deleting the ` + "`memory.json`" + ` data file.
